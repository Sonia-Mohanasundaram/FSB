...
import { Heart, LayoutDashboard, Stethoscope, CalendarCheck, Users, LogOut, Menu } from "lucide-react";
...
const HospitalAdminLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col md:flex-row">
      <header className="md:hidden sticky top-0 z-40 bg-background border-b border-border">
        <div className="h-14 px-4 flex items-center justify-between">
          <div className="flex items-center gap-2 min-w-0">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-purple-700 to-indigo-800 flex items-center justify-center shrink-0">
              <Heart className="h-5 w-5 text-white" />
            </div>
            <span className="font-semibold text-foreground truncate">MediCare Admin</span>
          </div>
      ...
      <aside className="hidden md:flex w-64 flex-col bg-gradient-to-b from-purple-700 to-indigo-800 text-white">
        <div className="p-6 flex items-center gap-2">
          <Heart className="h-6 w-6" />
          <span className="font-bold text-lg">MediCare Admin</span>
        </div>
